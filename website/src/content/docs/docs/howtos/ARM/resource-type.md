...
- **ManagedBy**: Indicating that a resource is managed by another Azure resource.
- **Extended Location**: Deploying a resource to an extended location such as an Azure Arc-connected cluster.
...
Support for deploying a resource to an extended location such as an Azure Arc-connected Kubernetes cluster or a custom location. To enable extended location support, add the `ExtendedLocationProperty` envelope property to the resource definition:
