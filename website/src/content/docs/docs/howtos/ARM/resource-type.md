...
- **ManagedBy**: Allowing management of this resource by another resource.
- **ExtendedLocation**: Specifying a custom or edge location for the resource.
...
Support for deploying a resource to a custom or edge location (such as Azure Arc-enabled Kubernetes clusters or Azure Edge Zones). To include extended location support, add the `ExtendedLocationProperty` standard envelope property to the resource definition:
