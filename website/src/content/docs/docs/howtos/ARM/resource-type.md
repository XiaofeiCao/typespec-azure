...
- **ManagedBy**: Indicating that the resource is managed by another resource or application.
- **ExtendedLocation**: Associating the resource with an extended location such as an Azure Edge Zone or a custom location.
...
Support for deploying a resource to an extended location such as an Azure Edge Zone or a custom location (e.g. Azure Arc). To enable extended location support, add the `ExtendedLocationProperty` envelope property to the resource definition:
...
For more information on extended locations, see [Extended Locations](https://learn.microsoft.com/azure/azure-arc/platform/conceptual-custom-locations)
