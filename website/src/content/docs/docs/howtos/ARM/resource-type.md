...
- **ManagedBy**: Indicating that a resource is managed by another resource.
- **ExtendedLocation**: Supporting deployment to Azure Extended Locations (Edge Zones, custom locations).
...
Support for deploying a resource to Azure Extended Locations such as Edge Zones or custom locations. To enable extended location support, add the `ExtendedLocationProperty` standard envelope property to the resource definition:
...
For more information on Azure Extended Locations, see [Extended Locations](https://learn.microsoft.com/azure/extended-zones/overview)
