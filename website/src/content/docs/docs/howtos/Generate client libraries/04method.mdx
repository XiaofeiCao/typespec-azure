...
For fine tuning, the set of decorators `@protocolAPI` and `@convenientAPI` can be used.
They take a boolean flag and should be scoped to `java`, `csharp`, or both, since those are
the emitters that consume them.
