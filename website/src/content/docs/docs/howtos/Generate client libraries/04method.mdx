...
For fine tuning, the set of decorators `@protocolAPI` and `@convenientAPI` can be used. They take a boolean flag and should be scoped to `"java"` and/or `"csharp"`, which are the emitters that currently use them.
