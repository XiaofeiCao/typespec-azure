...
func NewServiceClient(endpoint string, options *ServiceClientOptions) (*ServiceClient, error)

func (client *ServiceClient) Get(ctx context.Context, options *RequestOptions) (Response, error)

client, _ := NewServiceClient(endpoint, nil)
// client's version will be 2024-04-01

clientWithSpecifiedAPIVersion, _ := NewServiceClient(endpoint, &ServiceClientOptions{
	ClientOptions: azcore.ClientOptions{APIVersion: string(v2023_11_01)},
})
// client's version will be 2023-11-01
```
...
For Go clients, this client-level `APIVersion` override is also used when the detected API version parameter comes from a path segment instead of a query or header parameter.
