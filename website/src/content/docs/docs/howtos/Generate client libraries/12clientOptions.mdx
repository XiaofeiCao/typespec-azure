...
| `value`   | `unknown`                  | The option value. Literal values, TypeSpec model references, arrays, objects, and nested values are supported; the consuming emitter determines the expected shape. |
...
The decorator supports arbitrary values, including primitives, TypeSpec model references, arrays, objects, and nested combinations:
...
When the value references a TypeSpec model, the consuming emitter can resolve the corresponding SDK type and keep related customizations such as `@alternateType`.
