...
| `scope`   | `string` (optional)           | The target language scope. **Required in practice** - omitting scope or using an invalid scope produces an additional warning |
...
1. **Always specify a scope**: The decorator is designed for language-specific behavior. Omitting the scope or using an invalid scope produces an additional warning.
