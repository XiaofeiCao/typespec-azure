...
| `emitContentTypeHeader`   | `boolean`  | Operation                       | Go        | When `true`, Go keeps a literal `Content-Type` response header in the modeled response envelope for that operation instead of omitting it. |
...
```typespec
#suppress "@azure-tools/typespec-client-generator-core/client-option" "go response envelope should retain Content-Type"
@@clientOption(readWidget, "emitContentTypeHeader", true, "go");
