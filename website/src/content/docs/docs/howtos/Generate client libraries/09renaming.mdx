...
:::note
In the TypeScript emitter, a reserved-word operation inside an operation group is automatically renamed by appending the singularized group name. For example, `delete` in `Conversations` becomes `deleteConversation`. Use `@clientName(..., "javascript")` when you need to keep a different JavaScript-facing method name.
:::
